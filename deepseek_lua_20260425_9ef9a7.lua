...
-- Anti Aim
local CharAnti = require(game.ReplicatedStorage.Modules.Core.Char)
...
local Network2 = require(game.ReplicatedStorage.Modules.Core.Net)
...
RagdollModule.is_ragdolling.get = function(...)
    ...
    if result == true and _G.AntiRagdoll then
        ...
        Network2.send("end_ragdoll_early")
        Network2.send("clear_ragdoll")
    end
    return result
end
...
local function startSpectatorSpec()
    ...
    savedPlatformStand = humanoidSpec.PlatformStand
...
local function stopSpectatorSpec()
    freecam = false
    ...
    safeSet(function()
        if humanoidSpec and humanoidSpec.Parent then
            if savedPlatformStand ~= nil then
                humanoidSpec.PlatformStand = savedPlatformStand
            else
...
toggleBtnSpec.MouseButton1Click:Connect(function()
    if freecam then stopSpectatorSpec() else startSpectatorSpec() end
end)
...
player.CharacterAdded:Connect(function(c)
    if savedPlatformStand ~= nil and humanoidSpec and humanoidSpec.Parent then
        safeSet(function() humanoidSpec.PlatformStand = savedPlatformStand end)
        savedPlatformStand = nil
    end
    if next(savedPartAnchors) then restoreAllCharacterPartsSpec() end
    charSpec = c
    humanoidSpec = c:FindFirstChildOfClass("Humanoid")
    hrpSpec = c:FindFirstChild("HumanoidRootPart")
    if freecam then stopSpectatorSpec() end
end)

speedBoxSpec.PlaceholderText = tostring(SPEED)

print("✅ โหลดสคริปต์เรียบร้อยแล้ว")