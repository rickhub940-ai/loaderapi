...
local function startSpeed()
    ...
    speedConnection = RunService.RenderStepped:Connect(function()
        ...
        if humanoid and rootPart and humanoid.MoveDirection.Magnitude > 0 then
            ...
            rootPart.CFrame = rootPart.CFrame + (dir.Unit * (speedValue / 5))
        end
    end)
end
...
MiscTab:Toggle({
    ...
    Desc = "ข้ามแอนิเมชั่นการเปิดกล่อง",
    Icon = "gift",
    Default = false,
    Callback = function(v)
        EnabledSkip = v
...
-- รีเซ็ต speed connection เมื่อตัวละครเกิดใหม่
